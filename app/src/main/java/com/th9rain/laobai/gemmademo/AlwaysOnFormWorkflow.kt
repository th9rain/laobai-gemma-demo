...
object AlwaysOnFormWorkflow {
    ...
    fun openForm(state: FormDemoState): FormDemoState {
        return state.copy(
            ...
            steps = listOf(
                DemoStep("模拟打开表单", "当前页面：北京市朝阳区社区智慧课堂报名表"),
                DemoStep("Always-on Sentinel", "端侧识别到固定报名表模板，未上传原始屏幕"),
                DemoStep("轻量提醒", "我看到这是报名表，要不要帮您填写常用信息？"),
            )
        )
    }
    ...
    fun confirmFill(state: FormDemoState): FormDemoState {
        ...
        val steps = state.steps + listOf(
            ...
            DemoStep("本地记忆", "读取李桂兰的虚拟资料，敏感字段保持脱敏"),
            DemoStep("自动填写", "姓名、年龄段、手机号、住址、联系人、课程已填写", RiskLevel.Medium),
